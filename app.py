...
# ===== Merge strategy: normalize -> concat demuxer -> final compress =====
def make_intermediate(ffmpeg: str, in_path: str, out_path: str, height: int, fps: int, audio_kbps: int, has_audio: bool, dur: float, extra: list, progress_bar, log_area, pct_text, seg_start: float, seg_end: float):
    """
    Re-encode one clip to a uniform mezzanine:
      - scaled to target height (keep AR)
      - video H.264 veryfast CRF 18 (visually near-lossless but fast)
      - audio stereo 48k AAC (generate silence if missing)
      - constant FPS (fps)
    """
    if has_audio:
        filter_full = f"[0:v]scale=-2:{height}:flags=lanczos[v];[0:a]aresample=async=1:first_pts=0,aformat=sample_rates=48000:channel_layouts=stereo[a]"
        maps = ["-map","[v]","-map","[a]"]
        inputs = ["-i", in_path]
    else:
        # generate silence in graph for exact duration
        filter_full = f"[0:v]scale=-2:{height}:flags=lanczos[v];anullsrc=cl=stereo:r=48000,atrim=0:{dur:.3f},asetpts=N/SR/TB[a]"
        maps = ["-map","[v]","-map","[a]"]
        inputs = ["-i", in_path]
...
def concat_intermediates(ffmpeg: str, list_file: str, out_path: str, progress_bar, log_area, pct_text):
    # Concat demuxer is fast; we just give it a tiny progress bump
    cmd = [ffmpeg, "-y", "-f", "concat", "-safe", "0", "-i", list_file, "-c", "copy", "-movflags", "+faststart", out_path]
    run_with_progress(cmd, 1.0, 0.60, 0.68, progress_bar, log_area, pct_text)  # small slice of overall progress

def final_encode_size_cap(ffmpeg: str, in_path: str, out_path: str, vcodec: str, preset: str, video_bps: float, audio_kbps: int, total_dur: float, extra: list, progress_bar, log_area, pct_text):
...
# ===== Single pipeline (unchanged behavior) =====
def compress_single(ffmpeg: str, ffprobe: str, payload: dict, params: dict):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Build filter (video + audio or generated silence)
        if has_a:
            fc = f"[0:v]scale=-2:{params['max_height']}:flags=lanczos[v];[0:a]aresample=async=1:first_pts=0,aformat=sample_rates=48000:channel_layouts=stereo[a]"
        else:
            fc = f"[0:v]scale=-2:{params['max_height']}:flags=lanczos[v];anullsrc=cl=stereo:r=48000,atrim=0:{dur:.3f},asetpts=N/SR/TB[a]"
        ...
        if "Size Cap" in params["encode_mode"]:
            ...
            # pass1 (video only)
            cmd1 = [params["ffmpeg"], "-y", "-stats", "-i", str(in1),
                    "-filter_complex", fc, "-map", "[v]",
                    "-c:v", vcodec, "-preset", params["preset"],
                    "-b:v", str(int(video_bps)), "-maxrate", str(int(video_bps)), "-bufsize", str(int(video_bps*2)),
                    "-pass", "1", "-passlogfile", str(out_path)+ "_2passlog",
                    "-an", "-f", "mp4", os.devnull] + (params["extra"] or [])
            # pass2 (v+a)
            cmd2 = [params["ffmpeg"], "-y", "-stats", "-i", str(in1),
                    "-filter_complex", fc, "-map", "[v]", "-map", "[a]",
                    "-c:v", vcodec, "-preset", params["preset"],
                    "-b:v", str(int(video_bps)), "-maxrate", str(int(video_bps)), "-bufsize", str(int(video_bps*2)),
                    "-pix_fmt", "yuv420p", "-movflags", "+faststart",
                    "-c:a", "aac", "-b:a", f"{params['audio_kbps']}k",
                    "-pass", "2", "-passlogfile", str(out_path)+ "_2passlog",
            ...
            for ext in (".log", "-0.log", "-0.log.mbtree", ".mbtree"):
                f = str(out_path)+ "_2passlog" + ext
                if os.path.exists(f):
                    try: os.remove(f)
                    except: pass
        else:
            st.info(f"CRF: **{params['crf']}** (lower = higher quality & bigger file)")
            cmd = [params["ffmpeg"], "-y", "-stats", "-i", str(in1),
                   "-filter_complex", fc, "-map", "[v]", "-map", "[a]",
        ...
        st.download_button("⬇️ Download MP4", out_path.read_bytes(), file_name=params["out_name"], mime="video/mp4")

# ===== Merge pipeline (NEW: normalize → concat → final compress) =====
def merge_and_compress(ffmpeg: str, ffprobe: str, payloads: list, params: dict):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # 1) Normalize both to identical intermediates
        inter1 = tmpdir / "inter1.mp4"
        inter2 = tmpdir / "inter2.mp4"
        fps = 30  # unify FPS for concat demuxer
        ...
        # 3) Final compress to user target
        ...
        if "Size Cap" in params["encode_mode"]:
            ...
            final_encode_size_cap(params["ffmpeg"], str(merged_pre), str(final_path), vcodec, params["preset"], video_bps, params["audio_kbps"], total_dur, params["extra"], progress_bar, log_area, pct_text)
        else:
            st.info(f"CRF: **{params['crf']}** (lower = higher quality & bigger file)")
